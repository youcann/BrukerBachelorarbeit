...
\begin{itemize}
	...
	\end{equation}
	beschrieben werden. Dabei wird angenommen, dass eine 90\degree-Anregung stattgefunden hat, die $M_{z,eq}$ vollständig von der $z$-Achse in die $xy$-Ebene geklappt hat.
\end{itemize}
...
Die chemische Verschiebung führt meist zu Bildartefakten (vgl.  \autoref{sec:gewebeArtefakte}), kann aber auch zur Bilderzeugung genutzt werden. Dazu wird sie ortsaufgelöst gemessen. Dies wird als \textit{Chemical Shift Imaging (CSI)} bezeichnet \cite{Keevil2006}.
...
Eine modifizierte, schnellere Variante der Spinecho-Sequenz ist die Turbo-Spinecho-Sequenz. Je nach Hersteller des Tomographens wird sie auch als \textit{FSE} (\textit{Fast Spin Echo}, z.B. von GE verwendet) oder \textit{RARE} (\textit{Rapid Acquisition with Refocused Echoes}, z.B. von Bruker verwendet) bezeichnet.
...
Die Feldstärke des statischen Magnetfeldes ist bei präklinischen \gls{mrt}s in der Regel höher als bei klinischen: Durch die, im Vergleich zum Menschen, kleineren geometrischen Abmessungen der Tierorgane werden für die präklinische Bildgebung kleinere Voxelgrößen gefordert. Diese führen zu kleineren Signalstärke pro Voxel und damit bei gleichbleibendem Rauschen zu einem reduzierten Signal-zu-Rausch-Verhältnis (\gls{snr}).
...
Für eine Darstellung im Frequenzbereich wird die \textit{\gls{dft}} verwendet. Statt \gls{dft} wird häufig der Begriff \gls{fft} verwendet. Die \gls{fft} ist dabei lediglich eine schnelle Implementierung der \gls{dft}.
...
\begin{figure}[H]
	...
	\hfill
	\subcaptionbox{$|X_{norm}[f]|$}{\includegraphics[width=0.3\textwidth,height=0.3\textwidth]{plots/dftAbs.tikz}}
...
Zur Demonstration der Äquivalenz der beiden Verfahren wird ein komplexes Signal $S[n]$ der Länge $N$ betrachtet. $S[n]$ ergibt sich dabei mit $S_x=S_x[n]=cos(2\pi \nu_0 t)$ und $S_y=S_y[n]=sin(2\pi \nu_0 t)$ zu

\begin{equation}
	S[n]=S_x+j S_y.
\end{equation}

Weiterhin wird ein Tiefpass-gefilterter Phasenrauschprozess $\Delta t$ bzw. $\Delta \phi$ erzeugt. Dieser besitzt eine Gauß-förmige Wahrscheinlichkeitsdichtefunktion und einen $1/f$-förmiges Spektrum.
...
Eine einfache Möglichkeit für eine FR-Metrik ist die (zweidimensionale) \textit{mittlere quadratische Abweichung}, auch MSE (für eng. \textit{mean squared error}) genannt. Definiert ist das MSE zwischen einem $N \times M$ großen Bild $X$ und einem ebenso großen Referenzbild $R$ als \cite{Tan2013}:
